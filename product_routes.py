...
from schemas import OrderModel, ProductModel
...
async def list_all_product(Authorize: AuthJWT = Depends()):
    ...
    if current_user.is_staff:
        ...
        return HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only admin can see all product")
...
async def delete_product_by_id(id: int, Authorize: AuthJWT = Depends()):
    ...
    if current_user.is_staff:
        ...
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only SuperAdmin allowed to delete product")
...
async def update_product_by_id(id: int, update_data: ProductModel, Authorize: AuthJWT = Depends()):
    ...
    if current_user.is_staff:
        ...
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only SuperAdmin allowed to update product")